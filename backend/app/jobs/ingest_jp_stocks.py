...
def _jp_stooq_symbol(ticker: str) -> str:
    return f"{ticker.lower().replace('.t', '')}.jp"


async def ingest_jp_stocks(db: AsyncSession, trigger: str = "manual") -> dict:
    run = await create_source_run(db, "jp_multi_source", trigger)
    instruments = await get_instruments_by_market(db, "JP")
    ...
    await finish_source_run(db, run, status, total_rows, success, fail, progress={"current_source": "jp_multi_source"})
