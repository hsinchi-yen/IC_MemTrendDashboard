...
async def ingest_us_stocks(db: AsyncSession, trigger: str = "manual") -> dict:
    ...
    for instrument in instruments:
        ...
        source = None
        try:
            rows = await with_retries(lambda: fetch_finmind("USStockPrice", instrument.ticker, start_date, date.today()))
            source = "finmind"
        except Exception:
            try:
