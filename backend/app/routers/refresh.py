...
async def start_refresh(db: AsyncSession = Depends(get_db)) -> dict:
    ...
    task = asyncio.create_task(run_all_ingestion_jobs(trigger="manual"))
    await asyncio.sleep(0)
    return {"job_id": task.get_name() or "manual-refresh", "status": "started", "trigger": "manual"}
